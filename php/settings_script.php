<?php
    ...
    $query="SELECT password FROM users WHERE user_id='$user_id' AND email='$email' ";
    ...
    $query="UPDATE users SET password='$newpassword' INTO users WHERE user_id='$user_id' AND email='$email'";
    $query_result=mysqli_query($con,$query) or die(mysqli_error($con));
    }
  }
  else{
